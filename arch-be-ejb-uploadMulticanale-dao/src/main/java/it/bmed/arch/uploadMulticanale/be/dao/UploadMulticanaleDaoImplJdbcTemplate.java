...
public class UploadMulticanaleDaoImplJdbcTemplate implements UploadMulticanaleDaoJdbcTemplate, InitializingBean {
	...
	public ECMResponse insertMedia(ECMRequest parameters) throws ApplicationException, Exception {
		...
		try {
			log.debug("INSERT MEDIA >>>>>>>>>>>>>>>>>>", "insert into qpush_be.ECM_FILE ("
...
//		ecmFile.getNameApp()
		...
		StringBuilder queryStrBuilder = new StringBuilder()
				.append("SELECT * FROM qpush_be.ECM_FILE WHERE 1= 1 "
						+ whereClause);
		log.debug("LIST MEDIA QUERY CommandDataMove : {}", queryStrBuilder);
...
//		} 		
		...
		log.debug("UPDATE MEDIA - ID FILE ECM:", idFileECM);
		String Container_type = "" + request.getContainerType();
		log.debug("UPDATE MEDIA - CONTAINER TYPE:", Container_type);
		String NomeApp = "" + request.getNameApp();
		log.debug("UPDATE MEDIA - NAME APP:", NomeApp);
	...
	public ECMResponse selectMedia(Integer refId) throws ApplicationException, Exception {
		...
		try {
			...
			@SuppressWarnings({ "rawtypes", "unchecked" })
			ECMFile ret = getJdbcTemplate().queryForObject(query, new RowMapper<ECMFile>() {
						@Override
						public ECMFile mapRow(java.sql.ResultSet rs, int row) throws SQLException {
							// MAP YOUR FIELDS HERE							
	...
	@Override
	public ECMResponse listOlderMedia(int monthsAmount) throws ApplicationException, Exception {

		ECMResponse response = new ECMResponse();

		String query = "SELECT * FROM qpush_be.ECM_FILE WHERE COD_STATO_ECM<>4 AND GSTD_D_ULT_MODF_RECORD < add_months(TRUNC(sysdate, 'DDD'),-6)";
		
		log.debug("LIST OLDER MEDIA QUERY CommandDataMove : {}", query);

		try {

			@SuppressWarnings({ "rawtypes", "unchecked" })
			List<ECMFile> ret = getJdbcTemplate().query(query, new RowMapper() {
						@Override
						public Object mapRow(java.sql.ResultSet rs, int row) throws SQLException {
							// MAP YOUR FIELDS HERE							
							ECMFile te = new ECMFile();
